...
\newcommand{\footer}{ \vfill
  \mbox{}\hrulefill\\
  Upload your solutions until the 7.7.2016, 10h00 s.t. on
...
Arthur Allignol \hspace{\fill} 30.6.2016\\[-1.2ex]
\mbox{}\hrulefill\\
...
\begin{center}
\large{{\bf Exercise Sheet 8 --- Recursive Partitioning}}
\end{center}

\begin{aufgabe}{{\bf Titanic Passenger Survival}}

Consider the data set {\tt titanic.csv} that you analysed in exercise
sheet 6. The aim of this exercise is to compare the prediction error
you obtain on the test data {\tt titanic\_test.csv} using the logistic
model you fitted on exercise sheet 6 to trees and random forest. 

\begin{enumerate}
\item Impute missing ages and calculate the fare per person for {\tt
    titanic\_test.csv} as you did in exercise sheet 6.
\item Fit a regression tree to the original data {\tt titanic.csv}
  and prune it. Interpret the results.
\item Fit a random forest on {\tt titanic.csv}.
\item Compute the prediction error rates based on
  \begin{itemize}
  \item your best logistic model of sheet 6
  \item the classification tree
  \item the random forest
  \end{itemize}
  on the test data set {\tt titanic\_test.csv}. You may use the
  missclassication rate
  \begin{equation*}
    \frac{1}{n}\sum_{i = 1}^n (y_i \neq \hat{y}_i),
  \end{equation*}
  where $\hat{y}_i$ is the predicted survival status for individual
  $i$ and $y_i$ is the true survival status for individual $i$, $i =
  1, \dots, n$.
